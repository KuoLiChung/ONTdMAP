...
2. **Genomic Regions** (`.bed`):
   ...
   - All required files are provided in the [Releases](./releases/tag/v1.0).
...
```bash
   git clone ..git   # Clone the repository
   cd ONTdMAP
...
The reference files required for the pipeline (including promoter coordinates, CpG genomic positions, and ENCODE blacklists) are provided in the [Releases](./releases/tag/v1.0) section of this repository. Download the required files and define their absolute paths before executing the workflow.
   ```bash
...
The pipeline generates standardized visualizations across different modules. Example output plots can be found in the [example_plot](./example_plot) directory. Plots are grouped by their analytical category below.

1. **Data Distribution & Quality Control**
   
   ***Subcommand***: [`sample-qc`](./example_plot/sample-qc), [`group-qc`](./example_plot/group-qc)
...
2. **Spatial TSS Profiling**
   ...
   ***Subcommand***: [`sample-qc`](./example_plot/sample-qc), [`group-tss`](./example_plot/group-tss)
...
3. **Dimensionality Reduction & Clustering**
   ...
   ***Subcommand***: [`group-pca`](./example_plot/group-pca), [`dmr-pca`](./example_plot/dmr-pca)
...
4. **DMR Characterization & Annotation**
   ...
   ***Subcommand***: [`dmr-qc`](./example_plot/dmr-qc)
...
6. **Targeted Region Statistics**
   ...
   ***Subcommand***: [`box`](./example_plot/box)
...
## Prerequisites
An [environment.yml](./environment.yml) file is included in this repository to configure the required dependencies via Conda.
